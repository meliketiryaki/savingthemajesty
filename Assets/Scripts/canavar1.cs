...
public class canavar1 : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        {
            hareketYonu = -1;
            transform.localScale = new Vector3(4f, 4f, 4f); // Canavarı sola döndür
        }
        else if (transform.position.x <= sinirlamaSol && hareketYonu == -1)
        {
            hareketYonu = 1;
            transform.localScale = new Vector3(-4f, 4f, 4f); // Canavarı sağa döndür
        }
